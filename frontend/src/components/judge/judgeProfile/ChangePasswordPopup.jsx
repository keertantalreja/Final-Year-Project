...
const ChangePasswordPopup = ({ setIsPopupOpen, userId, userEmail, role }) => {
  ...
  const [validateJudgePassword] = useValidateJudgePasswordMutation();
  ...
  const [valiadtePassword] = useValidatePasswordMutation();
  const [resetPassword] = useResetPasswordMutation();
  const [loading, setLoading] = useState(false);
  const [isPasswordValidate, setIsPasswordValiadte] = useState(false);

  const [resetJudgePassword] = useResetJudgePasswordMutation();
  ...
  const validateUserPassword = async () => {
    setLoading(true);
    try {
      const result = await validateJudgePassword({
        id: userId,
  ...
  const resetUserPassword = async () => {
    setLoading(true);
    try {
      const result = await resetJudgePassword({
